...
class HTTPClient(object):
    ...
    def send_custom_comms(self, skill_key, data, no_response=False):
        ...
        """
        # Python3: Expecting data to be json encoded (must be of type byte, not str)
        rpc_request = {
            'data': base64.b64encode(data),
            'skill_key': skill_key,
            'no_response': no_response,  # this key is option and defaults to False
        }
        # Python3: We json encoded 'data', so it gets decoded here
        rpc_request['data'] = rpc_request['data'].decode()
