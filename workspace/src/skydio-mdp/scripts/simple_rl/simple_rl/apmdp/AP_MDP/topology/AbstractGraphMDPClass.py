...
class GraphL2State(State):
    ...
    def __eq__(self, other):
        return isinstance(other, GraphL2State) and self.agent_on_floor_number == other.agent_on_floor_number

class GraphL2GroundedAction(NonPrimitiveAbstractTask):
...
class GraphL1State(State):
    ...
    def __eq__(self, other):
        return isinstance(other, GraphL1State) and self.agent_in_room_number == other.agent_in_room_number

class GraphL1GroundedAction(NonPrimitiveAbstractTask):
    ...
    def _terminal_function(self, state):
        return self.l0_domain.get_room_numbers((state.x))[0] == self.goal_room
...
class GraphRootL2GroundedAction(RootTaskNode):
...
class GraphRootL1GroundedAction(RootTaskNode):
...
class GraphL2MDP(MDP):
    ...
    def __init__(self, starting_floor=1, gamma=0.99, env_file=[], constraints={}, ap_maps={}):
        ...
        if len(env_file) != 0:
            ...
            GraphL2MDP.ACTIONS = self.cube_env['L2ACTIONS']
        else:
            print("Input: env_file")

        initial_state = GraphL2State(starting_floor, self._transition_q(starting_floor, ""))
        if initial_state.q != 0:
            initial_state.set_terminal(True)

        MDP.__init__(self, GraphL2MDP.ACTIONS, self._transition_func, self._reward_func, init_state=initial_state,
                     gamma=gamma)
    ...
    def _transition_func(self, state, action):
        ...
        if abs(current_floor - action_floor_number) == 1:  # transition function
            ...
            next_state = GraphL2State(action_floor_number, next_q)
            if next_state.q != 0:
...
class GraphL1MDP(MDP):
    ...
    def __init__(self, starting_room=1, gamma=0.99, slip_prob=0.0, env_file=[], constraints = {}, ap_maps = {}):
        ...
        if len(env_file) != 0:
            self.cube_env = env_file[0]
            GraphL1MDP.ACTIONS = self.cube_env['L1ACTIONS']
        else:
            print("Input: env_file")

        initial_state = GraphL1State(starting_room, self._transition_q(starting_room, ""))
        if initial_state.q != 0:
            initial_state.set_terminal(True)

        MDP.__init__(self, GraphL1MDP.ACTIONS, self._transition_func, self._reward_func, init_state=initial_state,
                     gamma=gamma)
    ...
    def _transition_func(self, state, action):
        ...
        if current_room in self.cube_env['transition_table'].keys():
            if action_room_number in self.cube_env['transition_table'][current_room]:
                ...
                next_state = GraphL1State(action_room_number, next_q)
...
#    @classmethod
    def action_for_room_number(self, room_number):
        for action in GraphL1MDP.ACTIONS:
...
def debug_l1_grid_world():
    def get_l1_policy(start_room=None, goal_room=None, mdp=None):
        if mdp is None:
            mdp = GraphL1MDP(start_room, goal_room)
...
if __name__ == '__main__':
    mdp = GraphL1MDP(env_file=[build_cube_env()])
    print('done')