...
from simple_rl.apmdp.AP_MDP.topology.GraphMDPClass import GraphMDP
from simple_rl.apmdp.AP_MDP.topology.AbstractGraphMDPClass import GraphL1MDP, GraphL2MDP


class GraphL2PolicyGenerator(AMDPPolicyGenerator):
    ...
    def generate_policy(self, l2_state, grounded_action):
        ...
        '''
        mdp = GraphL2MDP(l2_state.agent_on_floor_number, env_file=self.env_file,
...
class GraphL1PolicyGenerator(AMDPPolicyGenerator):
    ...
    def generate_policy(self, l1_state, grounded_action):
        ...
        mdp = GraphL1MDP(l1_state.agent_in_room_number, env_file=self.env_file,
                        constraints=grounded_action.goal_constraints,
                        ap_maps=grounded_action.ap_maps)
        return self.get_policy(mdp, verbose=self.verbose, max_iterations=50, horizon=100)
...
class GraphL0PolicyGenerator(AMDPPolicyGenerator):
...
#        destination_locations = self.domain.room_to_locs[grounded_task.goal_state.agent_in_room_number]
        init_location = (state.x)
        mdp = GraphMDP(init_loc=init_location, env_file=self.env_file,
